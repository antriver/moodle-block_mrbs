...
require_capability('block/mrbs:bookvehicles', context_system::instance());

$thisurl = new moodle_url('/blocks/mrbs/web/edit_vehicle_entry.php', array('day' => $day, 'month' => $month, 'year' => $year));

if ($area) {
    $thisurl->param('area', $area);
} else {
    $area = MRBS_VEHICLE_AREA;
}
...
if ($id > 0) {
    ...
    $passengers = intval($entry->passengers);
...
redirect_to_booking_form($area, true);
...
// Get the first vehicle by default
if ($room_id == 0) {
    $dbroom = $DB->get_records('block_mrbs_room', array('area_id' => $area), 'room_name', 'id', 0, 1);
    if ($dbroom) {
        $dbroom = reset($dbroom);
        $room_id = $dbroom->id;
    }
...
redirect_to_booking_form($area_id, true);

// determine if there is more than one area
$areas = $DB->get_records('block_mrbs_area', array('id' => $area_id), 'area_name');
...
$PAGE->requires->js('/blocks/mrbs/web/updatefreerooms.js?v=2', true);
...
var on_vehicle_page = true;
    ...
    function validate_and_submit() {
        ...
        if ((!i1 || (i1 && i2)) && document.forms["main"].rep_type && document.forms["main"].rep_type[6].checked && (!n || n < 2)) {
            alert("Please enter a number of weeks to repeat for");
            return false;
        }
        ...
        // Vehicle validation
        if (!$('textarea[name=destination]').val()) {
            alert('Please enter a destination.');
            return false;
        }

        if (!$('select[name=passengers]').val() || $('select[name=passengers]').val() == '0') {
...
<H2><?=($id ? 'Edit A Vehicle Booking' : 'Book A Vehicle')?></H2>

<FORM NAME="main" ACTION="edit_entry_handler.php" METHOD="GET">
<input type="hidden" name="sesskey" value="<?php echo sesskey(); ?>">

<TABLE BORDER=0 id="mrbs_form">

<?php if ($edit_type != 'series' && $rep_id) { ?>
    <tr>
        <td colspan="2">
            <b><?php $editseriesurl = new moodle_url('/blocks/mrbs/web/edit_entry.php', array('id' => $id, 'edit_type' => 'series'));
...
<TR>
    <TD CLASS=CR><B>Reservation for (DragonNet username):</B></TD>
    <TD CLASS=CL><INPUT NAME="name" SIZE=40 VALUE="<?php echo htmlspecialchars($name, ENT_NOQUOTES) ?>"></TD>
</TR>

<TR>
    <TD CLASS=TR><B>Purpose and any special requirements:</B></TD>
...
<tr>
    ...
    <td>
        <textarea name="destination" rows="8" cols="40"><?= htmlspecialchars($destination) ?></textarea>
    </td>
</tr>
...
<?php if (!$enable_periods) { ?>
    <TR>
        ...
        <TD CLASS=CL><INPUT NAME="hour" SIZE=2 VALUE="<?php if (!$twentyfourhour_format && ($start_hour > 12)) {
                ...
                echo $start_hour;
            } ?>" MAXLENGTH=2 onChange="updateFreeRooms()">:<INPUT NAME="minute" SIZE=2
                                                                                                                             VALUE="<?php echo $start_min; ?>" MAXLENGTH=2
                                                                                                                             onChange="updateFreeRooms()">
            <?php
...
<TR>
    ...
    <TD CLASS=CL><INPUT NAME="duration" SIZE=7 VALUE="<?php echo $duration; ?>" onChange="updateFreeRooms()">
        ...
        <INPUT NAME="all_day" TYPE="checkbox" VALUE="yes" id="all_day"
                     <?php if ($all_day) echo 'CHECKED '; ?>onClick="OnAllDayClick()"> <?php echo get_string('all_day', 'block_mrbs');
...
?>
<tr>
    <td class=CR><b>Vehicle:</b></td>
    <td class=CL valign=top>
        <table>
            <tr>
                <td><select name="rooms[]" multiple="yes" class="roomSelector">
                        <?php
                ...
                <td><?php echo get_string('ctrl_click_vehicle', 'block_mrbs') ?></td>
            </tr>
            <tr>
                <td><label for="nooccupied">Hide vehicles that are not available <input
                        name="nooccupied" id="nooccupied" type="checkbox" checked="checked"
                        onclick="updateFreeRooms()"/></label></td>
                <td></td>
            </tr>
...
<tr>
    <td class="TR">
        <strong>Total number of passengers:</strong>
    </td>
    <td>
        <select name="passengers" class="passengerSelector" data-value="<?=$passengers?>">
            <option value="0">Please select vehicle(s) first</option>
        </select>
    </td>
</tr>
...
function setMaxPassengers(max) {

    var val = parseInt($('.passengerSelector').attr('data-value'));
    $('.passengerSelector').html('<option></option>');
    for (i = 1; i <= max; i++) {
        $('.passengerSelector').append('<option ' + (i == val ? 'selected="selected"' : '') + '>' + i + '</option>');
    }
...
{
    // Set total number of passenger spaces when changing "room"
    var totalCapacity = 0;
    var regex = new RegExp(/Capacity\:([0-9]+)/);
    $('.roomSelector').children(':selected').each(function(){
        if (capacity = $(this).text().match(regex)) {
            totalCapacity += parseInt(capacity[1]);
        }
    });

    // Update passenger selector
    setMaxPassengers(totalCapacity);
}

$(document).on('change', '.passengerSelector', function(){
    $(this).attr('data-value', $(this).val());
});

$(document).on('change', '.roomSelector', updatePassengerCapacity);
$(document).ready(updatePassengerCapacity);
